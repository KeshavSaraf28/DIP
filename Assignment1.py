...
def bilinear_interpolation(img, scale):
    ...
    # Visualization of pixel value calculation
    #         X1
    # A •-----•--------• B
    #         |
    #         • Y
    ...
    for i in range(1, height+1):
        for j in range(1, width+1):
            ...
            # then wrt those points (X1, X2) finding vertical point using linear interpolation
            ...
            pixel_value = ((a*(1-x_weight) + b*x_weight) * (1-y_weight)
                           ) + ((c*(1-x_weight) + d*x_weight) * y_weight)
...
# scaled_dim1 = (math.ceil(scale_factor * img_width),
#                math.ceil(scale_factor * img_height))
# scaled_img_builtin1 = cv2.resize(
#     img, scaled_dim1, interpolation=cv2.INTER_CUBIC)
# cv2.imshow("Scaled Image using Built-in Functions (scale=1)",
#            scaled_img_builtin1)
...
# Using built-in function
scaled_dim2 = (math.ceil(scale_factor * img_width),
               math.ceil(scale_factor * img_height))
scaled_img_builtin2 = cv2.resize(
    img, scaled_dim2, interpolation=cv2.INTER_AREA)
cv2.imshow("Scaled Image using Built-in Functions (scale=2)",
           scaled_img_builtin2)
...
# Using built-in function
scaled_dim3 = (math.ceil(scale_factor * img_width),
               math.ceil(scale_factor * img_height))
scaled_img_builtin3 = cv2.resize(
    img, scaled_dim3, interpolation=cv2.INTER_AREA)
cv2.imshow("Scaled Image using Built-in Functions (scale=0.5)",
           scaled_img_builtin3)
diff = cv2.subtract(scaled_img_builtin3, scaled_img3)
cv2.imshow("Scaled Image using difference", diff)
